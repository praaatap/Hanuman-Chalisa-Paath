...
class _ReadingScreenState extends State<ReadingScreen> {
  ...
  String _getLocalizedTitle(AppSettings settings) {
    if (_currentPage >= chalisaData.length) {
      return AppStrings.get('hanuman_chalisa_completed', settings.appLanguage);
    }
    final verse = chalisaData[_currentPage];
    String typeDisplay = verse.type;
    if (verse.type.contains(' ')) {
      final parts = verse.type.split(' ');
      final typeKey = parts[0].toLowerCase();
      final typeName = AppStrings.get(typeKey, settings.appLanguage);
      typeDisplay = "$typeName ${parts[1]}";
    } else {
      final typeKey = verse.type.toLowerCase();
      typeDisplay = AppStrings.get(typeKey, settings.appLanguage);
    }
    return typeDisplay;
  }
                      ...
                      child: Container(
                        ...
                        child: PageFlipWidget(
                          ...
                          children: List.generate(
                            chalisaData.length,
                            (index) => _buildPage(index, settings),
                          ),
                        ),
                      ),
                    ),
  ...
  Widget _buildPage(int index, AppSettings settings) {
    const Color backgroundLight = Color(0xFFF9F8F6);
        ...
        children: [
          ...
          if (index == 0)
            Padding(
              ...
              child: Text(
                AppStrings.get('swipe_to_start_reading', settings.appLanguage),
                textAlign: TextAlign.center,
                style: GoogleFonts.manrope(
                  textStyle: TextStyle(
